...
class b4st_walker_nav_menu extends Walker_Nav_menu
{
	...
	function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0, $object = null, $current_object_id = 0)
	{ // li a span
		// $allowed_categories = array_merge(["public"], array_map(function ($category) {
		// 	return $category["slug"];
		// }, get_categories(["meta_key" => "slug", "meta_compare" => "LIKE", "meta_value" => "public-%"])));
		// if (is_user_logged_in()) {
		// 	$allowed_categories = array_merge($allowed_categories, ["protected"]);
		// 	$current_user = wp_get_current_user();
		// 	foreach ($current_user->roles as $role) {
		// 		$allowed_categories = array_merge($allowed_categories, [
		// 			"public-" . (explode("_", $role)[0]),
		// 			"protected-" . (explode("_", $role)[0])
		// 		]);
		// 	}
		// }
		...
		// 	error_log("RETURNING" . $id . print_r($allowed_categories, true) . "!=" . print_r($item->object_id, true));
		// 	return;
		// }
